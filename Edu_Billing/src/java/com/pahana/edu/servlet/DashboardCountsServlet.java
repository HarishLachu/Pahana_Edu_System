...
public class DashboardCountsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/pahana_edu1", "root", "");

            // Total Bills
            PreparedStatement ps1 = conn.prepareStatement("SELECT COUNT(*) AS count FROM bills");
            ResultSet rs1 = ps1.executeQuery();
            if (rs1.next()) {
                json.addProperty("bills", rs1.getInt("count"));
            }
            rs1.close();
            ps1.close();
            ...
            if (rs2.next()) {
                json.addProperty("customers", rs2.getInt("count"));
            }
            rs2.close();
            ps2.close();
            ...
            if (rs3.next()) {
                json.addProperty("items", rs3.getInt("count"));
            }
            rs3.close();
            ps3.close();
